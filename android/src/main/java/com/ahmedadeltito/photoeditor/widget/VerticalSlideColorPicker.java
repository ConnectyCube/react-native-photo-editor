...
//        }
    ...
    private void moveToStartColor() {
        if (startColor == 0) return;

        for (int i = (int) colorPickerBody.top; i <= colorPickerBody.bottom; i++) {
    ...
    private int getSelectorBorderColor() {
        return colorPrimary;
    }

    private Paint getSelectorBorderPaint() {
        if (selectorBorderPaint == null) {
            selectorBorderPaint = new Paint();
            selectorBorderPaint.setColor(getSelectorBorderColor());
            selectorBorderPaint.setStrokeWidth(selectorBorderWidth);
            selectorBorderPaint.setStyle(Paint.Style.STROKE);
            selectorBorderPaint.setAntiAlias(true);
            selectorBorderPaint.setDither(true);
        }
